...
var worker_default = {
  async fetch(request, env) {
    ...
    try {
      ...
      const tokens = excludeToken ? allTokens.filter((t) => t.token !== excludeToken) : allTokens;
      const results = await Promise.allSettled(
        tokens.map(
          ({ token, platform }) => sendToToken({ title, body: notifBody, accessToken, projectId, token, platform })
        )
      );
...
var ICON_URL = "https://ipuc-pis-directory.web.app/icons/Icon-512.png";
async function sendToToken({ title, body, accessToken, projectId, token, platform }) {
  const isWeb = platform === "web";
  const message = {
    token,
    notification: { title, body },
    ...isWeb ? { webpush: { notification: { icon: ICON_URL, badge: ICON_URL, image: ICON_URL } } } : {
      android: {
...
__name(sendToToken, "sendToToken");
